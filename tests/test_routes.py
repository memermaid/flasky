...
def test_get_all_cats_with_populated_db_returns_list_json(client, seven_cats):
    response = client.get('/cats')
    response_body = response.get_json()
...
def test_post_one_cat_creates_cat_in_db(client):
    ...
    assert response_body == {'id': 1,
                            'msg': 'Successfully created a cat with id 1'}
...
def test_get_one_cat_with_empty_db_returns_404(client):
    ...
    assert response_body == {"msg": "Could not find cat with id 1"}